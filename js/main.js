...
function displayTodos() {
  ...
  todos.forEach((todo, index) => {
    let timeDiff = new Date(todo.dueDate).getTime() - new Date().getTime();
    let secondsDiff = timeDiff / 1000;
    console.log(secondsDiff.toFixed(0) > 0);
    if (secondsDiff.toFixed(0) > 0) {
      output += `
...
function submitTodo(e) {
  ...
  if (
    new Date(dueDateInput.value).toLocaleDateString() >
  ...
  if (messages.size === 0) {
    let dueDate = new Date(dueDateInput.value).toLocaleDateString("fr", {
      weekday: "long",
      year: "numeric",
      month: "long",
      day: "numeric"
...
function editTodo(e) {
  ...
  if (e.target.id === "editBtn") {
    nameInput.value =
      e.target.parentElement.previousElementSibling.previousElementSibling.children[0].textContent;
...
function cancelUpdate(e) {
  ...
  messages.clear();
  changeCurrentState("startMode");
  messages.add(["success", "modification abandonée"]);
  showAlert(messages);
  clearInputs();
}
